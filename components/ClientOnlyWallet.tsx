...
export function ClientOnlyWallet() {
  ...
  if (!farcasterUser?.fid) {
    return (
      <div className="flex items-center justify-center border-2 border-black text-black text-xs px-2 py-1 rounded-md font-medium"
      ...
      >
        {farcasterUser?.pfpUrl ? (
          <img 
            ...
            alt={farcasterUser.displayName || farcasterUser.username || `FID: ${farcasterUser.fid}`}
          ...
          <div className="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center text-xs border border-gray-300">
            {farcasterUser?.username?.substring(0, 1) || farcasterUser?.displayName?.substring(0, 1) || '?'}
          </div>
        )}
      </button>

      {showDropdown && (
        <div className="absolute right-0 top-10 bg-white border-2 border-black rounded-md min-w-48 z-50" style={{boxShadow: '4px 4px 0px rgba(0,0,0,0.3)'}}>
          <div className="p-3">
